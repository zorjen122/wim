...
- Auth Gate HTTP adapter；Connection Gateway TCP/TLV/Protobuf 登录、心跳、重连、
  退避、请求超时和同类请求本地背压。
...
当前仅完成本地假 Gate/Gateway 自动化测试，尚未对正在测试中的真实服务端执行联调。
...
提供伪实现。文件上传和部分群能力已有 C++ 请求接口，但尚未形成完整媒体/群资料 UI。
...
工程使用 Qt 的跨平台 executable/QML module 入口，Android 构建时会保留 QML
import scan。目前本机仅安装了 Linux Qt 6.10.2 kit，没有 Android Qt kit、SDK/NDK，
所以 Android 空壳尚未在本机交叉编译验证。
