...
> 状态：执行基线 v1.7（UI 第一阶段在当前公开协议边界内完成；DATA-1～DATA-2、Linux 通知和网络阶段一已落地；核心单聊真实联调已验证）
> 日期：2026-07-18
...
当前实现已提供“全部 / 未读 / 群聊”筛选、按名称和摘要搜索、筛选空状态以及清除条件
动作。筛选层保留源模型行号，选择会话、置顶、静音和标记已读不会因可视列表下标变化而
作用到错误会话。“新建会话”统一进入联系人流程，不保留无响应的工具栏按钮。
...
当前会话内搜索已复用消息模型的可视代理，按正文即时过滤并提供结果数、无结果空状态
和清除动作；关闭搜索后恢复原时间线位置。消息模型保留服务端原始时间串并派生时间文本、
日期标签、日期分隔和首条未读位置，SQLite 无需为纯展示字段升级 schema。

现有公开拉取契约只有 `after_seq` 向前增量同步，没有 `before_seq` 或更早页游标，因此
“加载更早消息”不能用现有接口诚实实现。待服务端补反向历史游标后再增加顶部加载入口；
当前客户端仍会按 `after_seq + has_more + next_seq` 自动拉完同步缺口。
...
当前已落地桌面右键、hover 更多按钮和移动长按底部操作面板；复制写入系统剪贴板，
可重试失败使用原 `client_message_id` 重新发送。回复在协议和本地模型支持前显示为禁用
占位，删除本地仍待 repository 语义确定后实现；附件入口同样以禁用占位明确表达边界。
...
申请已从联系人页内嵌标签拆为独立顶层入口，导航徽标显示待处理数量；页面区分待处理和
历史记录。联系人页保留用户资料、发消息、添加好友、创建群和申请入群，并支持按姓名、
状态或用户 ID 搜索。接受/忽略请求进入独立提交中状态，按钮不可重复点击；业务或传输
失败恢复为待处理，成功结果同时更新模型、快照和 repository。

Fake Repository 模式不会再因缺少网络而拒绝添加好友、创建群或申请入群：前两者给出
确定性操作反馈，创建群还会生成并打开本地群会话，保证 UI-3 可脱离服务端演示。
...
当前会话详情会按 `group:` 会话类型切换为群聊资料，避免显示单聊的“最近在线”，并提供
明确的群成员列表占位。成员数据和管理动作在稳定接口出现前不使用伪造的服务结果。
...
截至本版，UI-0～UI-3 已在当前公开协议边界内完成，DATA-1～DATA-2、NET-1 的公开
协议适配和 Linux 桌面入口也已落地。Android arm64 工具链、APK 构建、USB 安装和
真机冷启动已验证，不再列为待办。新增完成的 UI 闭环包括独立申请入口及待处理徽标、
全局连接状态条、会话/联系人/会话内搜索、会话筛选、日期与未读分隔、设置和群资料
骨架，以及消息右键/长按、移动底部操作面板、复制和同 ID 重试。离屏 smoke 现在还会
断言关键列表确实含有内容，不再只验证进程未崩溃。接下来按以下顺序推进：

1. 在已验证安装和冷启动的 arm64 真机上完成双向文本、断线补同步与重启恢复验收。
2. 完善并验证 Android 系统返回、safe area、软键盘和应用前后台恢复；开发服务器地址
   入口已完成。
3. 服务端提供反向历史游标后补分页时间线和“加载更早消息”；这不再作为当前 UI
   第一阶段的伪占位功能。
4. 资料编辑、完整群成员、回复和本地删除等待领域及服务契约稳定后进入后续里程碑。
5. 服务端测试稳定后补双 Gateway 故障注入；客户端并行推进 Linux 托盘。
