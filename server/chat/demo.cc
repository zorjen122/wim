...
int main() {
    try {
        ...
        // 检查 "Host" 键是否存在
        if (!gateway["Host"]) {
            throw std::runtime_error("config.yaml: 'Host' key not found in 'Gateway' node");
        }

        // 检查 "Port" 键是否存在
        if (!gateway["Port"]) {
            throw std::runtime_error("config.yaml: 'Port' key not found in 'Gateway' node");
        }

        // 输出 Gateway 的 Host 和 Port
        std::cout << "Gateway host: " << gateway["Host"].as<std::string>() << std::endl;
