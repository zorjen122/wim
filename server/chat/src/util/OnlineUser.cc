...
OnlineUser::~OnlineUser() {
  ...
  for (auto &[_, uidTimers] : waitAckTimerMap) {
    for (auto &[__, timer] : uidTimers) {
      if (timer)
        timer->cancel();
    }
  }
  waitAckTimerMap.clear();
  sessionMap.clear();
}
// userId:machineId
...
void OnlineUser::cancelAckTimer(long seq, long uid) {
  ...
  {
    std::lock_guard<std::mutex> lock(sessionMutex);
    auto seqIter = waitAckTimerMap.find(seq);
    if (seqIter == waitAckTimerMap.end())
      return;

    auto uidIter = seqIter->second.find(uid);
    if (uidIter == seqIter->second.end())
      return;
...
void OnlineUser::onReWrite(OnlineUser::ReWriteType type, long seq, long uid,
  ...
  waitAckTimer->async_wait([this, type, seq, uid, session, packet, rsp,
    ...
    if (ec == boost::asio::error::operation_aborted) {
      ...
      if (session->IsConnected()) {
        ...
        LOG_DEBUG(wim::businessLogger,
                  ...
                  "refcount: {}",
                  seq, callcount, short(type), session.use_count());
        // 对于心跳类型重传，seq为用户Id
        if (type == ReWriteType::HeartBeat) {
          ClearUser(uid, uid, session);
        } else if (type == ReWriteType::Message) {
...
void OnlineUser::Pong(long uid) {
  ...
  ChatSession::Ptr session = GetUserSession(uid);
  if (session == nullptr) {
    LOG_WARN(businessLogger, "用户不在线，无法发送Pong, uid: {}", uid);
    return;
  }
