...
uint32_t Tlv::getDataSize() {
  return total - PROTOCOL_HEADER_TOTAL;
}
uint32_t Tlv::getTotal() {
  return total;
}

std::string Tlv::getData() {
  return std::string(data, total);
}
...
std::string Channel::getData() {
  return protocolData->getData();
};
...
void ChatSession::DoSend(std::string msgData, uint32_t msgID) {
  ...
  if (senderSize > PROTOCOL_QUEUE_MAX_SIZE) {
    LOG_ERROR(netLogger,
              "会话ID：{}的消息被丢弃，因其承载已超出最大发送队列数量, "
              "当前大小({}) > "
              "最大发送队列数量({})",
              sessionId, senderSize, PROTOCOL_QUEUE_MAX_SIZE);
    return;
  }
...
void ChatSession::HandleWrite(const net::error_code &ec,
  ...
  try {
    ...
    LOG_ERROR(netLogger, "发送失败，异常信息： {}", e.what());
  }
}
...
net::io_context &ChatSession::GetIoc() {
  return ioContext;
}

size_t ChatSession::GetSessionID() {
  return sessionId;
}
